...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function createRowCard(cells, index) {
        ...
        generateBtn.onclick = () => {
            // Lógica condicional de Origem com base em "PC Configuration Location"
            let source_from = '';
            let source_cep = '';
            let source_adress = '';
            let source_UF = '';

            const loc = (dataObj['PC Configuration Location'] || '').trim().toLowerCase();
            if (loc.includes('armazém 09') || loc.includes('armazem 09') || loc.includes('armazém 9') || loc.includes('armazem 9') || loc === 'armazém 09' || loc === 'armazem 09') {
                source_from = "Recife - PE (Armazém 9)";
                source_cep = "50.030-150";
                source_adress = "Av. Alfredo Lisboa , S/N Armazém 9- Bairro Recife_ PE";
                source_UF = "PE";
            } else if (loc.includes('fortaleza')) {
                source_from = "Fortaleza - CE (Scopa Platinum Corporate)";
                source_cep = "60115-191";
                source_adress = "R. Monsenhor Bruno, 1153 - G4 - Aldeota";
                source_UF = "CE";
            } else if (loc.includes('campina grande')) {
                source_from = "Campina Grande - PB (Unifacisa)";
                source_cep = "58411-020";
                source_adress = "Unifacisa Building 7 - Av. Sen. Argemiro de Figueiredo, 1901 - Sandra Cavalcante";
                source_UF = "PB";
            ...
            // Lógica condicional de Modelo do Asset com base em "SERIAL"
            let model = '';
            const serial = (dataObj['SERIAL'] || '').trim();
            if (serial.startsWith('PE0')) {
                model = "Lenovo Thinkpad T14";
            } else if (serial.startsWith('5CD')) {
                model = "HP EliteBook EB6G1i14";
            } else if (/^\d/.test(serial) && !serial.startsWith('5CD')) {
                model = "Dell Inc. Latitude 5420";
            }

            // Template com a substituição das variáveis
            const template = `Item: Laptop
Description: Boa tarde, tudo bem? Precisamos da NF desta maquina para transporte de ${source_UF} para ${dataObj['State/Province of residence'] || ''}, ${dataObj['Worker (Full Name)'] || ''}
